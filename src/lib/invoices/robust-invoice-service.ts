...
async function generateInvoice(request: InvoiceGenerationRequest): Promise<InvoiceGenerationResult> {
  try {
    ...
    const invoiceData = {
      ...
      paymentDetails: {
        freelancerAmount: Math.round((amount * (1 - 0.052666)) * 100) / 100, // 5.2666% platform fee
        platformFee: Math.round((amount * 0.052666) * 100) / 100
      },
...
async function calculateInvoiceAmount(
  ...
  try {
    ...
    if (request.invoiceType === 'completion') {
      ...
      console.log(`💰 Completion-based calculation for project ${project.projectId}:`, {
        totalBudget,
        upfrontCommitment,
        remainingBudget,
        totalTasks,
      ...
      console.log(`💰 Milestone-based calculation for project ${project.projectId}:`, {
        totalBudget,
        totalTasks,
        amountPerMilestone,
