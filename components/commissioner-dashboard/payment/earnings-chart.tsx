...
export default function EarningsChart({ range }: Props) {
  const [dailyTotals, setDailyTotals] = useState<DailyEarnings[]>([]);
  const [yMax, setYMax] = useState(1000);

  useEffect(() => {
    async function fetchSummary() {
      try {
        const res = await fetch(`/api/commissioner-dashboard/payments/earnings-summary?range=${range}`);
        const data: EarningsSummaryResponse = await res.json();

        if (Array.isArray(data.dailyBreakdown)) {
          const sorted = data.dailyBreakdown.sort(
            (a: DailyEarnings, b: DailyEarnings) =>
        ...
        console.error('Failed to load earnings summary for range:', range, err);
      }
    }
