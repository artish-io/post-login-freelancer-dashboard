...
export default function RevenueChart({ range }: Props) {
  const [dailyTotals, setDailyTotals] = useState<DailyRevenue[]>([]);
...
const getYMax = (maxAmount: number) => {
  ...
  useEffect(() => {
    async function fetchSummary() {
      try {
        const res = await fetch(`/api/commissioner-dashboard/payments/revenue-summary?range=${range}`);
        const data: RevenueSummaryResponse = await res.json();

        setHasProducts(data.hasProducts);

        if (data.hasProducts && Array.isArray(data.dailyBreakdown)) {
          const sorted = data.dailyBreakdown.sort(
            (a: DailyRevenue, b: DailyRevenue) =>
              new Date(a.date).getTime() - new Date(b.date).getTime()
          );
          const max = Math.max(...sorted.map((d: DailyRevenue) => d.amount));
          setYMax(getYMax(max));
        ...
        console.error('Failed to load revenue summary for range:', range, err);
      }
    }
      ...
      <div className="w-full">
        ...
        <div className="h-64 flex flex-col items-center justify-center text-gray-500">
          <div className="text-center">
            <p className="mb-2">No digital products found</p>
            <p className="text-sm">Create products in your storefront to track revenue</p>
          </div>
        </div>
      </div>
    );
