...
export default function ProposalMilestoneItem({ data, onUpdate, onRemove }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3 border border-gray-300 rounded-2xl p-5 bg-white relative">
      ...
      <div className="flex flex-col md:flex-row gap-3">
        {renderDatePicker('Start date', data.startDate, (val) => onUpdate({ startDate: val }))}
        {renderDatePicker('End date', data.endDate, (val) => onUpdate({ endDate: val }), data.startDate)}
      </div>
    </div>
  );
}